...
// !!!!!!! Endpoint público para obtener peluqueros (accesible sin autenticación)
server.get('/peluqueros', async (req, res) => {
  try {
    const peluqueros = await database.all(
      `SELECT id, nombre, apellido, email, telefono FROM usuarios WHERE rol = ?`,
